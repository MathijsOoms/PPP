...
const getCountry = function (countryName) {
    const request = handleFetchData(`https://restcountries.com/v3.1/name/${countryName}`, 'Country not found!')
        .then((data) => {
            showHtml(data[0]);
            const borderCountries = data[0].borders;
            // const borderCountries = ['jshbvd'];
            if (!borderCountries) throw new Error('There is no border country!');
            return handleFetchData(`https://restcountries.com/v3.1/alpha?codes=${borderCountries}`, 'Country not found!')
        })
